...
프로젝트의 출발점은 관심 기업에서 같은 세부 사업을 하는 기업과 공급망으로 연결된 기업을 쉽게 찾으려는 필요입니다. 루멘텀에서 코히런트 같은 광통신 기업을 찾는 경우와 Apple에서 아이폰 공급업체를 찾는 경우를 동기로 설명합니다. 산업분류의 세부 정보 부족, 공시를 일일이 읽는 수고, 관심 기업의 공시에 상대 이름이 없을 수 있다는 문제를 사업 유사도와 여러 기업의 공시 언급 분석으로 연결합니다. 이 예시는 탐색 동기를 설명하며, 해당 기업쌍의 앱 검색 순위나 주가 상승 예측 실적을 주장하지 않습니다.
