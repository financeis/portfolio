...
전체 목록은 처음에 대표 프로젝트 3개만 표시하고, ‘전체 프로젝트 9개 보기’로 전체 목록을 펼칩니다. 분류 탭은 항상 표시하며 ‘금융·AI 활용 프로젝트’와 ‘대회 및 수상’을 선택하면 해당 분류의 항목을 모두 표시합니다. ‘전체’ 탭으로 돌아오거나 ‘프로젝트 접기’를 누르면 대표 프로젝트 3개만 표시합니다. 대표 프로젝트는 금융 리서치 프로젝트로 표시하며, `featuredOrder`에 따라 Research Desk → 10-K Report → Mini-Factory 순서를 고정합니다. 세 카드에는 녹색·청색·황토색의 옅은 배경을 적용합니다. 카드의 설명·주제·링크와 상세 내용은 유지합니다.

- 분류: `finance_ai` (금융·AI 활용 프로젝트), `award` (대회 및 수상). 대회·수상 항목은 `award`로 분류합니다.
